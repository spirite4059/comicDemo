...
@Controller
@RequestMapping("/admin/publicCai")
public class PublicCaiController extends BaseController
{
    @Autowired
    private PublicCaiService publicCaiService;
    ...
    public Map<String,Object>publicCaiSave(PublicCai cai){
        ...
        try{
                int option_num=0;
                if(cai.getOp1() == null ||"".equals(cai.getOp1())) option_num+=1;
                if(cai.getOp2() == null ||"".equals(cai.getOp1())) option_num+=1;
                if(cai.getOp3() == null ||"".equals(cai.getOp1())) option_num+=1;
                if(cai.getOp4() == null ||"".equals(cai.getOp1())) option_num+=1;
                if(cai.getOp5() == null ||"".equals(cai.getOp1())) option_num+=1;
                if(cai.getOp6() == null ||"".equals(cai.getOp1())) option_num+=1;

                cai.setCreatDate(new Date());
                cai.setOp1Num(0);cai.setOp2Num(0);cai.setOp3Num(0);cai.setOp4Num(0);cai.setOp5Num(0);cai.setOp6Num(0);
                cai.setResultOpNum(-1);
                cai.setState(1);
                cai.setUseOpNum(option_num); //计算出共使用了几个option，用来做选择
